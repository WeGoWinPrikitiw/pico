...
export function useTopUp() {
    ...
    return useMutation({
        mutationFn: async (params: { userPrincipal: string; amount: number }) => {
            return await operationalService.topUp(params.userPrincipal, params.amount);
        },
...
export function useBuyNFT() {
    ...
    return useMutation({
        mutationFn: async (params: { buyer: string; seller: string; nftId: number; price: number; forumId?: number }) => {
            return await operationalService.buyNFT(params.buyer, params.seller, params.nftId, params.price, params.forumId);
        },
        onSuccess: (_, variables) => {
            ...
            queryClient.invalidateQueries({ queryKey: createQueryKey.nft(variables.nftId) });
        },
        onError: (error: Error) => {
...
export function useApproveContract() {
    return useMutation({
        mutationFn: async () => {
            throw new Error("Approval must be done through the ledger canister directly. Use the getApprovalInfo method to get the correct parameters.");
        },
        onError: (error: Error) => {
            console.error("Contract approval failed:", error);
            toast.error("Contract approval requires direct ledger interaction. Check documentation for details.");
        },
    });
} 