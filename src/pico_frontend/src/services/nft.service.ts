...
import { NFT, TransferArgs } from "../../../declarations/nft_contract/nft_contract.did";
import {
    idlFactory as nftIdlFactory
} from "../../../declarations/nft_contract";
import { HttpAgent, Identity } from "@dfinity/agent";

export class NFTService extends BaseService {
    private get nftActor(): NFT {
        return this.createActor<NFT>(
            import.meta.env.CANISTER_ID_NFT_CONTRACT || "rrkah-fqaaa-aaaaa-aaaaq-cai",
            nftIdlFactory
        );
    }
    ...
    async getNFTsByTrait(traitType: string, traitValue: string): Promise<FrontendNFTInfo[]> {
        try {
            const result = await this.nftActor.get_nfts_by_trait(traitType, traitValue);
            return result.map(nft => this.convertNFTInfo(nft));
        } catch (error) {
            this.handleError(error);
        }
    ...
    async getOwnerOf(tokenIds: number[]): Promise<(Account | null)[]> {
        try {
            ...
            return result.map(owner => (owner.length > 0 && owner[0]) ? owner[0] : null);
        } catch (error) {
            this.handleError(error);
        }
    }
