...
export function NFTDetailPage() {
    ...
    const { data: userBalance = 0, isLoading: isLoadingBalance } =
    ...
    const handleApprovalFlow = () => {
        ...
        if (!nft || !principal || !tokenInfo)
            return toast.error("NFT data not available.");
    ...
    const handlePurchase = () => {
        ...
        if (principal === String(nft.owner))
            return toast.error("You already own this NFT.");
    ...
    const handleShare = async () => {
        const shareUrl = `${window.location.origin}/share/nft/${nftId}`;
        try {
    ...
    const hasInsufficientFunds =
        isAuthenticated && !isOwner && userBalance < Number(nft.price);
                        ...
                        <div className="flex flex-col space-y-6">
                            ...
                            <Card className="bg-muted/40">
                                ...
                                <CardContent>
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        {isAuthenticated && (
                                            <div className="text-right">
                                                ...
                                                <p
                                                    className={`text-lg font-semibold ${hasInsufficientFunds
                                                        ? "text-destructive"
                                                        : "text-foreground"
                                                        }`}
                                                >
                                                    {isLoadingBalance ? (
                                                        ...
                                                        `${(userBalance / 1e8).toFixed(2)} PiCO`
                                                    )}
                                                </p>
                                            </div>
                                        )}
                                    ...
                                    {isOwner ? (
                                        <div className="p-3 bg-background rounded-lg text-center text-sm text-muted-foreground">
                                            You own this NFT.
                                        </div>
                                    ) : !isAuthenticated ? (
                                        ...
                                        <Button
                                            ...
                                            disabled={purchaseMutation.isPending || isLoadingApproval}
                                        >
                                            {isLoadingApproval ? (
                            ...
                            >
                                <DialogContent className="sm:max-w-[425px]">
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>
                                            To purchase this NFT, you need to approve the contract to
                                            spend your PiCO tokens.
                                        </DialogDescription>
                                    </DialogHeader>
                                    <div className="py-4 space-y-4">
                                        ...
                                        <div className="flex justify-between items-center text-sm font-semibold">
                                            ...
                                            <span className="text-primary">
                                                {((nft.price + 1e6) / 1e8).toFixed(2)} PiCO
                                            </span>
                                        </div>
                                        <div className="p-3 bg-muted rounded-lg text-xs text-muted-foreground">
                                            <p>
                                                This approval allows the contract to spend up to{" "}
                                                {((nft.price + 1e6) / 1e8).toFixed(2)} PiCO tokens from
                                                your wallet. The extra 0.01 PiCO buffer ensures
                                                ...
                                                <div className="space-y-6">
                                                    {isAuthenticated ? (
                                                        <div className="flex items-start gap-4">
                                                            <UserAvatar principalId={principal} size="lg" />
                                                    ...
                                                    <div className="space-y-6">
                                                        <h3 className="font-semibold text-lg">
                                                            Comments ({mainForum.comments.length})
                                                        </h3>
                                                        {isLoadingForums ? (
                                            ...
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm text-foreground">
                                                        <UserName principalId={String(nft.owner)} currentUser={principal} maxLength={12} />
                                                    </span>
                                                </div>
                                            </div>
                                            <Separator />
                                            ...
                                            <div className="space-y-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-muted-foreground">
                                                            by {String(nft.owner).slice(0, 6)}... on{" "}
                                                            {formatDate(nft.created_at)}
                                                        </p>
                                                    </div>
                                                </div>
