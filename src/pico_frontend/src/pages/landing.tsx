...
export function LandingPage() {
  ...
  const floatingElements = [
    {
      className: "absolute top-1/4 left-1/6 w-2 h-2 sm:w-3 sm:h-3 lg:w-4 lg:h-4 bg-fuchsia-400 rounded-full animate-ping",
      delay: "0.5s"
    },
    {
      className: "absolute top-2/3 right-1/4 w-2 h-2 sm:w-2 sm:h-2 lg:w-3 lg:h-3 bg-lime-400 rounded-full animate-ping",
      delay: "1s"
    },
    {
    ...
    {
      className: "absolute top-3/4 left-1/3 w-2 h-2 sm:w-3 sm:h-3 lg:w-4 lg:h-4 bg-cyan-400 rounded-full animate-ping",
      delay: "2s"
    }
  ];
    ...
    <div className="relative bg-[#0C031A] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center gap-4 sm:gap-6 md:gap-8 lg:gap-10 xl:gap-12 max-w-full sm:max-w-2xl md:max-w-4xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl mx-auto text-center px-4 sm:px-6 md:px-8 pb-8 sm:pb-12 md:pb-16 lg:pb-20">
        ...
        <p className="text-sm text-balance sm:text-base md:text-lg lg:text-xl xl:text-2xl text-[#E7D8FFA8] max-w-full sm:max-w-lg md:max-w-2xl lg:max-w-3xl xl:max-w-4xl leading-relaxed px-2 sm:px-0">
          The ultimate art marketplace where creativity meets the blockchain. Mint your artwork as NFTs, showcase it, and connect with collectors <span className="font-black">who truly get it.</span>
        </p>

        {/* CTA Button */}
        <Button className="flex text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl w-auto h-auto px-4 py-2 sm:px-6 sm:py-3 md:px-8 md:py-4 lg:px-10 lg:py-5 justify-center items-center gap-2 rounded-full border border-white/37 bg-[rgba(255,158,251,0.23)] text-white hover:bg-[rgba(255,158,251,0.35)] transition-all duration-300 hover:scale-105 backdrop-blur-sm min-w-max">
        ...
        {mounted && (
          <>
            ...
            <div className="hidden md:block">
              <div className="absolute top-1/6 right-1/6 w-2 h-2 lg:w-3 lg:h-3 xl:w-4 xl:h-4 bg-yellow-400 rounded-full animate-ping" style={{ animationDelay: "2.5s", animationDuration: "4s" }} />
              <div className="absolute bottom-1/4 left-1/5 w-2 h-2 lg:w-3 lg:h-3 xl:w-4 xl:h-4 bg-blue-400 rounded-full animate-ping" style={{ animationDelay: "3s", animationDuration: "3.5s" }} />
            </div>

            {/* Extra floating elements for very large screens */}
            <div className="hidden xl:block">
              <div className="absolute top-1/3 right-1/3 w-3 h-3 2xl:w-4 2xl:h-4 bg-pink-400 rounded-full animate-ping" style={{ animationDelay: "4s", animationDuration: "3s" }} />
              <div className="absolute bottom-1/3 right-1/5 w-2 h-2 2xl:w-3 2xl:h-3 bg-green-400 rounded-full animate-ping" style={{ animationDelay: "4.5s", animationDuration: "3.5s" }} />
            </div>
          </>
        )}
      </div>
