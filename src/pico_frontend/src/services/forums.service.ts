...
export interface ForumsActor {
    createForum: (input: CreateForumInput) => Promise<{ ok: Forum } | { err: string }>;
    getAllForums: () => Promise<Forum[]>;
    getForumsByUser: (principalId: string) => Promise<Forum[]>;
    likeForum: (forumId: bigint, userId: string) => Promise<{ ok: Forum } | { err: string }>;
    commentForum: (forumId: bigint, comment: string, userId: string) => Promise<{ ok: Forum } | { err: string }>;
...
export class ForumsService extends BaseService {
    ...
    async commentForum(forumId: number, comment: string, userId: string): Promise<Forum> {
        try {
            const result = await this.forumsActor.commentForum(BigInt(forumId), comment, userId);
            return this.handleResult(result);
        } catch (error) {
            this.handleError(error);
        }
