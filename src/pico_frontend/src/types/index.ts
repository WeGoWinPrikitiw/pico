...
export interface ContentService {
    uploadContent: (data: Uint8Array, contentType: string) => Promise<ContractResult<string>>;
    ...
    shareContent: (contentId: string, targetUser: string) => Promise<ContractResult<string>>;
}
...
export const QueryKeys = {
    ...
    NFTS_BY_TRAIT: (type: string, value: string) => ["nfts", "trait", type, value] as const,
    NFTS_BY_RARITY: (rarity: string) => ["nfts", "rarity", rarity] as const,

    // Operational
    TRANSACTIONS: ["transactions"] as const,
    USER_TRANSACTIONS: (principal: string) => ["transactions", "user", principal] as const,
    ...
    USER_PREFERENCES: (principal: string) => ["preferences", "user", principal] as const,
...
export interface FrontendContentData extends Omit<ContentData, 'created_at' | 'updated_at'> {
    created_at: number;
    updated_at: number;
}
