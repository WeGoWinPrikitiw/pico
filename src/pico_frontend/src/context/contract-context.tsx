...
export function ContractProvider({ children }: ContractProviderProps) {
    ...
    const tokenBalance = useTokenBalance(userAccount!, isAuthenticated && !!userAccount);
    ...
    const contextValue = useMemo<ContractContextValue>(() => ({
        ...
        token: {
            ...
            balance: {
                ...
                formatted: tokenInfo.data && tokenBalance.data ?
                    tokenHelpers.formatTokens(tokenBalance.data, tokenInfo.data.decimals) :
                    undefined,
            },
...
export function useCreatePreferencesWithErrorHandling() {
    ...
    return useCallback((input: PreferencesInput) => {
        try {
...
export function useTransferTokenWithErrorHandling() {
    ...
    return useCallback((args: TransferArg) => {
        try {
...
export function useApproveTokenWithErrorHandling() {
    ...
    return useCallback((args: ApproveArgs) => {
        try {
...
export function useUserPreferencesWithFallback(fallbackPreferences: string[] = []) {
    const { preferences } = useContractContext();
...
export function useTokenBalanceFormatted() {
    ...
    return useMemo(() => {
        if (token.balance.data && token.info.data) {
            return token.helpers.formatTokens(token.balance.data, token.info.data.decimals);
        }
        return '0';
...
export function useIsContractReady() {
    ...
    return useMemo(() => {
        return isInitialized && 
               ...
               !!token.info.data;
    }, [isInitialized, preferences.isLoading, token.info.isLoading, token.info.data]);
}