...
import type { Account, Tokens, TransferArg, ApproveArgs, AllowanceArgs } from "@/types";
...
export function useTokenBalance(account: Account, enabled: boolean = true) {
    return useQuery({
        queryKey: [...createQueryKey.all(), "icrc1", "balance", account.owner.toString()],
        queryFn: async () => {
...
export function useAllowance(args: AllowanceArgs, enabled: boolean = true) {
    return useQuery({
        queryKey: [...createQueryKey.all(), "icrc1", "allowance", args.account.owner.toString(), args.spender.owner.toString()],
        queryFn: async () => {
            const service = serviceFactory.getICRC1Service();
...
export function useTokenHelpers() {
    ...
    const createAccount = (principalText: string, subaccount?: Uint8Array): Account => {
    ...
    const createApproveArgs = (
        ...
        const parsedExpectedAllowance = expectedAllowance ? 
            service.parseTokens(expectedAllowance, decimals) : undefined;
...
export function useUserTokenDashboard(principalText?: string) {
    ...
    const data = {
        ...
        formattedBalance: tokenInfo.data && balance.data ? 
            serviceFactory.getICRC1Service().formatTokens(balance.data, tokenInfo.data.decimals) : 
            undefined,
    };
