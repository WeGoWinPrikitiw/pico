...
export function useUsersByPreference(preference: string, enabled: boolean = true) {
    return useQuery({
...
export function useHasPreferences(principalId: string, enabled: boolean = true) {
    return useQuery({
...
export function useAddPreference() {
    ...
    return useMutation({
        mutationFn: async ({ principalId, preference }: { principalId: string; preference: string }) => {
            const service = serviceFactory.getPreferencesService();
