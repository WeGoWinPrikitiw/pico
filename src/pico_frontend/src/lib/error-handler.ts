...
export class ErrorHandler {
    ...
    private static categorizeError(message: string, code?: string): ErrorType {
        ...
        if (code?.includes("AUTH") || lowerMessage.includes("authentication") || lowerMessage.includes("identity")) {
            return ErrorType.AUTHENTICATION;
        }

        if (code?.includes("PERMISSION") || lowerMessage.includes("unauthorized") || lowerMessage.includes("forbidden")) {
        ...
        if (code?.includes("CONTRACT") || lowerMessage.includes("canister") || lowerMessage.includes("contract")) {
    ...
    private static isRetryable(code?: string): boolean {
        const retryableCodes = ["NETWORK_ERROR", "TIMEOUT", "TEMPORARILY_UNAVAILABLE", "429"];
    ...
    static handleError(
        ...
        const contractError = this.createError(error, operation, contractMethod, userAction, principal);
        ...
        try {
            ...
            if (result instanceof Promise) {
                return result.catch((error) => {
                    ErrorHandler.handleError(error, true, operation, contractMethod, userAction);
                    throw error;
                });
            }
...
export function useErrorReporting() {
    const reportError = (error: ContractError, additionalContext?: Record<string, any>) => {
