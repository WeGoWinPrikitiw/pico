...
export class PreferencesService extends BaseService {
    ...
    private initializeActor() {
        ...
        const canisterId = import.meta.env.VITE_CANISTER_ID_PREFERENCES_CONTRACT ||
    ...
    private ensureActor() {
        if (!this.actor) {
            throw new Error("Preferences service not initialized. Please authenticate first.");
        }
        return this.actor;
    }
    ...
    async addPreference(principalId: string, preference: string): Promise<UserPreferences> {
        try {
            const actor = this.ensureActor();
    ...
    async removePreference(principalId: string, preference: string): Promise<UserPreferences> {
        try {
            const actor = this.ensureActor();
    ...
    convertStatsToFrontend(stats: {
        ...
        return {
            ...
            average_preferences_per_user: this.convertBigIntToNumber(stats.average_preferences_per_user),
        };
    }
