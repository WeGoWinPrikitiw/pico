...
export class ICRC1Service extends BaseService {
    ...
    private initializeActor() {
        ...
        const canisterId = import.meta.env.VITE_CANISTER_ID_ICRC1_LEDGER ||
            import.meta.env.CANISTER_ID_ICRC1_LEDGER;
    ...
    private ensureActor() {
        if (!this.actor) {
            throw new Error("ICRC1 service not initialized. Please authenticate first.");
        }
        return this.actor;
    }
    ...
    createTransferArgs(to: Account, amount: Tokens, memo?: Uint8Array, fee?: Tokens): TransferArg {
        return {
            to,
            amount,
            memo: memo ? [memo] : [],
