...
class BackendService {
  ...
  private async initializeActors() {
    try {
      ...
      this.operationalV3Actor = createOperationalV3Actor(
        operationalV3CanisterId,
        {
          agent: this.agent,
        },
      );

      this.operationalV2Actor = createOperationalV2Actor(
        operationalV2CanisterId,
        {
          agent: this.agent,
        },
      );
  ...
  async getUserBalance(principalId?: string): Promise<UserBalance | null> {
    try {
      ...
      const result =
        await this.operationalV3Actor.getUserCompleteBalance(principal);
      if ("ok" in result) {
        return result.ok;
      } else {
  ...
  async depositToEscrow(
    ...
    try {
      ...
      const result = await this.operationalV3Actor.deposit_to_escrow(
        principalId,
        amount,
      );
      if ("ok" in result) {
  ...
  async withdrawFromEscrow(
    ...
    try {
      ...
      const result = await this.operationalV3Actor.withdraw_from_escrow(
        principalId,
        amount,
      );
      if ("ok" in result) {
  ...
  async mintTokens(
    ...
    try {
      ...
      const result = await this.operationalV3Actor.mint_tokens(
        toPrincipal,
        amount,
      );
      if ("ok" in result) {
  ...
  async getUserTransactions(principalId?: string): Promise<Transaction[]> {
    try {
      ...
      const transactions =
        await this.operationalV3Actor.getUserTransactions(principal);
      return transactions
        .map((tx) =>
          tx
            ? {
                ...
                nft_id:
                  tx.nft_id && tx.nft_id.length > 0 ? tx.nft_id[0] : undefined,
                status: tx.status
                  ? (Object.keys(tx.status)[0] as any)
                  : "Pending",
                forum_id:
                  tx.forum_id && tx.forum_id.length > 0
                    ? tx.forum_id[0]
                    : undefined,
  ...
  async getTransaction(transactionId: bigint): Promise<Transaction | null> {
    try {
      ...
      const result =
        await this.operationalV3Actor.getTransaction(transactionId);
      if (result.length > 0) {
        const tx = result[0];
        return tx
          ? {
  ...
  async getNFTs(): Promise<NFT[]> {
    ...
    const mockNFTs: NFT[] = [
      ...
      },
      {
        ...
        image:
          "https://images.unsplash.com/photo-1561998338-13ad7883b20f?w=500",
