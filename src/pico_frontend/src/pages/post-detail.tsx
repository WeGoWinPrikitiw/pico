...
const defaultPost: PostDetail = {
    ...
    history: [
        { id: '1', type: 'list', price: '20 PiCO', timestamp: '2024-03-14 10:00:00' },
        { id: '2', type: 'sale', price: '25.5 PiCO', timestamp: '2024-03-15 11:00:00' }
    ],
    isForSale: true,
    saleEnds: '2024-04-14',
    comments: 45
    ...
    const getMockData = (id: string): PostDetail => {
        if (id === 'mock') {
            ...
            const buildPostFromTx = (): PostDetail => {
                const tx = transactions.find((t: any) => t.nft_id && t.nft_id.toString() === id);
                if (tx) {
                    ...
                    return {
                        ...
                        image: imageFallback,
                        creator: {
                            id: tx.from_principal_id ? `${tx.from_principal_id.slice(0, 8)}...` : 'Unknown',
                            name: tx.from_principal_id ? `${tx.from_principal_id.slice(0, 8)}...` : 'Unknown',
                        ...
                        isLiked: false,
                        isBookmarked: false,
                        createdAt: new Date(Number(tx.created_at) / 1000000).toLocaleDateString(),
                        tags: [],
                        category: 'NFT',
                        royalty: 0,
                        blockchain: 'Internet Computer',
                ...
                return {
                    ...
                    history: [
                        { id: '1', type: 'created', price: '25.5 PiCO', timestamp: '2024-03-15 10:30:00' },
                        { id: '2', type: 'listed', price: '25.5 PiCO', timestamp: '2024-03-15 11:00:00' },
                    ],
                    isForSale: false,
                    saleEnds: '',
                    comments: 0
            ...
            const handleBuyNFT = async () => {
                ...
                try {
                    await buyNFT(
                        principal,
                        transactions.find((t: any) => t.nft_id && t.nft_id.toString() === id)?.from_principal_id || '',
                        post.id,
                        post.price
                    );
                } catch (error) {
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                                <div>
                                                    ...
                                                    <p className="text-lg font-bold text-primary">{post.price} PiCO</p>
                                                </div>
                                                <div>
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                                    {post.properties.map((prop) => (
                                                        <div key={prop.trait_type} className="p-3 bg-muted rounded-lg">
                                                            <p className="text-xs text-muted-foreground uppercase">{prop.trait_type}</p>
                                                            <p className="font-medium truncate">{prop.value}</p>
                                                            <p className="text-xs text-muted-foreground mt-1">{prop.rarity}% have this trait</p>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                            ...
                                            {post.history.map((tx) => (
                                                <div key={tx.id} className="flex items-center justify-between p-4 bg-muted rounded-lg">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <p className="font-medium">{tx.type === 'sale' ? 'Sold' : 'Listed'}</p>
                                    ...
                                    <CardContent className="p-6">
                                        ...
                                        <div className="flex gap-3 mb-6">
                                            {post.creator.website && (
                                                <a href={post.creator.website} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground">
                                                    <Globe className="h-5 w-5" />
                                                </a>
                                            )}
                                            {post.creator.social?.twitter && (
                                                <a href={`https://twitter.com/${post.creator.social.twitter}`} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground">
                                                    <Twitter className="h-5 w-5" />
                                                </a>
                                            )}
                                            {post.creator.social?.instagram && (
                                                <a href={`https://instagram.com/${post.creator.social.instagram}`} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground">
                                        ...
                                        <div className="grid grid-cols-3 gap-4 text-center mb-6">
                                            <div>
                                                <p className="text-2xl font-bold">{post.creator.followers}</p>
                                        ...
                                        <CardContent className="p-6">
                                            <div className="space-y-6">
                                                <div>
                                                    ...
                                                    <p className="text-3xl font-bold text-primary">{post.price} PiCO</p>
                                                </div>
                                                ...
                                                <div className="p-4 bg-muted rounded-lg">
                                                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                                        ...
                                                        <p>Sale ends {new Date(post.saleEnds).toLocaleDateString()}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </CardContent>
